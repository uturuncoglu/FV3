      module mdul_sfc_diff
      use mdul_sfc_sice, only: cimin

      contains
      subroutine sfc_diff_ocean(im,ps,u1,v1,t1,q1,z1,
...
!
     ...
     &                    ps,u1,v1,t1,q1,z1,snwdph,tskin,
     &                    prsl1,prslki,ddvel,sigmaf,shdmax,tsurf,
     &                    covice
      real(kind=kind_phys), dimension(im), intent(INOUT) :: z0rl
      real(kind=kind_phys), dimension(im), intent(OUT)   ::
     &                    cm,ch,rb,stress,fm,fh,ustar,wind,fm10,fh2

      integer, dimension(im),intent(IN) ::   vegtype

      logical, intent(IN) ::   flag_iter(im) ! added by s.lu
...
!           z0 = arnu / (ustar(i) * ff ** pp)
      ...
      return
      end subroutine sfc_diff_ocean

      subroutine sfc_diff_land(im,ps,u1,v1,t1,q1,z1,
     &                    snwdph,tskin,z0rl,cm,ch,rb,
     &                    prsl1,prslki,
     &                    idry,
     &                    stress,fm,fh,
...
!
      ...
      real(kind=kind_phys), dimension(im), intent(IN)    ::
     &                    ps,u1,v1,t1,q1,z1,snwdph,tskin,z0rl,
     &                    prsl1,prslki,ddvel,sigmaf,shdmax,tsurf,
     &                    z0pert,ztpert ! mg, sfc-perts

      real(kind=kind_phys), dimension(im), intent(INOUT) :: ustar
      real(kind=kind_phys), dimension(im), intent(OUT)   ::
     &                    cm,ch,rb,stress,fm,fh,wind,fm10,fh2
...
!
      do i=1,im
       if( idry(i) == 1) then          ! Flag for some land at point
        if(flag_iter(i)) then 
          wind(i) = max(sqrt(u1(i)*u1(i) + v1(i)*v1(i))
     &                + max(0.0, min(ddvel(i), 30.0)), 1.0)
          tem1    = 1.0 + rvrdm1 * max(q1(i),1.e-8)
...
!
      ...
      return
      end subroutine sfc_diff_land

      subroutine sfc_diff_ice(im,ps,u1,v1,t1,q1,z1,
     &                    snwdph,tskin,z0rl,cm,ch,rb,
     &                    prsl1,prslki,fice,
     &                    stress,fm,fh,
     &                    ustar,wind,ddvel,fm10,fh2,
...
!
      ...
      real(kind=kind_phys), dimension(im), intent(IN)    ::
     &                    ps,u1,v1,t1,q1,z1,snwdph,tskin,z0rl,
     &                    prsl1,prslki,ddvel,sigmaf,shdmax,tsurf,fice
      real(kind=kind_phys), dimension(im), intent(INOUT) :: ustar
      real(kind=kind_phys), dimension(im), intent(OUT)   ::
...
!
      do i=1,im
!      if (islimsk(i).eq.2) then	! ice point
       if (fice(i) >= cimin) then	! ice point
        if(flag_iter(i)) then 
          wind(i) = max(sqrt(u1(i)*u1(i) + v1(i)*v1(i))
     &                + max(0.0, min(ddvel(i), 30.0)), 1.0)
          tem1    = 1.0 + rvrdm1 * max(q1(i),1.e-8)
...
!
      ...
      enddo
      return
      end subroutine sfc_diff_ice
      end module mdul_sfc_diff
